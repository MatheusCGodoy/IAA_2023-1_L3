...
for problem in fileList:
    ...
    result_mine = os.popen('./fast-downward/fast-downward.py' + ' castle/domain.pddl' + ' castle/' + problem + ' --search "eager_greedy([planopt_ff()])"').read()
    ...
    print('\n' + result_mine)
    with open('out_' + problem.split('.')[0] + '_ff.txt', 'w') as file:
